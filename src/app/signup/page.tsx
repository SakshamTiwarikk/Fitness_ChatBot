...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (signInRes?.error) { setError("Account created but sign-in failed. Please log in."); setLoading(false); return; }
    ...
    <div className="min-h-screen bg-bg flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-surface border border-border rounded-2xl p-8">
          <h1 className="font-barlow text-2xl font-bold tracking-wide mb-2">START YOUR JOURNEY</h1>
        ...
        <p className="text-center text-muted text-sm mt-4">
          ...
          <Link href="/login" className="text-lime hover:underline">Sign in</Link>
        </p>
      </div>
    </div>
  );
